(() => {
  ...
  const toast = (m) => {
    ...
    d.className = "toast";
    d.textContent = m;
    app.querySelector(".shell")?.appendChild(d);
    setTimeout(() => d.remove(), 2200);
  };

  const icon = (x) => `<span>${x}</span>`;
  ...
  function layout(inner, nav = true) {
    return `<div class="app"><div class="shell">${inner}${
      nav ? navBar() : ""
    }</div></div>`;
  }
  ...
  function navBar() {
    ...
    return `<div class="nav">${n
      .map(
        (x) =>
          `<button class="${
            state.screen === x[0] ? "active" : ""
          }" onclick="go('${x[0]}')">${icon(
            x[1]
          )}<b>${x[2]}</b></button>`
      )
  ...
  function renderAuth() {
    app.innerHTML = layout(
      `<div class="screen center" style="justify-content:center;padding:28px">
        <img class="logo" src="./logo.svg">
        <h1>Leo Chat</h1>
        <p class="muted">Chat. Connect. Roar.</p>
        ...
        <button class="btn" onclick="auth('signin')">
          Sign in
        </button>

        <button class="btn secondary" onclick="auth('signup')">
          Create account
        </button>
      </div>`,
      false
  ...
  function renderSetup() {
    app.innerHTML = layout(
      `<div class="screen center" style="justify-content:center;padding:25px">
        <img class="logo" src="./logo.svg">

        <h2>Set up your Leo profile</h2>
  ...
  window.saveProfile = async () => {
    ...
    if (!username || !display_name) {
      return toast("Complete your profile");
    }
    ...
    app.innerHTML = layout(
      `<div class="screen">
        <div class="top">
          ...
          <button
            class="iconbtn"
            onclick="state.screen='notifications';render()"
          >
            🔔
          </button>
        </div>
  ...
  async function filterPeople() {
    ...
    const arr = state.profiles.filter(
      (p) =>
        p.id !== state.user.id &&
  ...
  window.sendPhoto = async (e) => {
    ...
    if (f.size > 4 * 1024 * 1024) {
      return toast("Photo must be under 4 MB");
    }
    ...
    app.innerHTML = layout(
      `<div class="screen">
        ...
        <div class="top">
          ...
          <button
            class="iconbtn"
            onclick="document.getElementById('momentFile').click()"
          >
            ＋
          </button>
      ...
      `<div class="screen">
        ...
        <div class="content">
          ...
          <div class="card center">
            ...
            <p class="muted">
              Voice and video call controls are ready.
              A production call service/WebRTC connection
              is required for live audio and video.
            </p>

            <button
              class="btn"
              onclick="toast('Choose a person from Chats to call')"
            >
              Start a call
            </button>
  ...
  function settingRow(k, label) {
    return `<div
      class="row"
      style="padding:10px 0;border-bottom:1px solid #222"
    >
      <div class="grow">
        ${label}
      </div>
      ...
      <input
        class="switch"
        type="checkbox"
        ${state.settings[k] ? "checked" : ""}
        onchange="toggleSetting('${k}',this.checked)"
      >
    </div>`;
  }

  window.toggleSetting = (k, v) => {
    state.settings[k] = v;
